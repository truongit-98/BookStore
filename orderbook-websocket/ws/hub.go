...
func addConn(hub *HubModel, register *Subscription) {
	...
	if (strings.Contains(register.Channel, "WS_EVENT_UPDATE_ORDER_TRANSACTION") ||
		strings.Contains(register.Channel, "WS_EVENT_PUBLIC_ORDER_BOOK")) && !isExist {
...
func checkChannelIsAllow(channel string) bool {
	...
	if strings.Contains(channel, "WS_EVENT_PUBLIC_ORDER_BOOK") || strings.Contains(channel, "WS_EVENT_UPDATE_ORDER_TRANSACTION") {
		return true
	}
